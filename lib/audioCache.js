...
export function hashText(text, voiceId, sig) {
  let key = text.trim().toLowerCase();
  if (voiceId && voiceId !== DEFAULT_VOICE_ID) key = `${voiceId}|${key}`;
  if (sig) key = `${key}|s:${sig}`;
  return crypto.createHash('sha256').update(key, 'utf8').digest('hex');
}

export async function getCachedAudioUrl(text, voiceId, sig) {
  try {
    const supabase = getSupabaseAdmin();
    const hash = hashText(text, voiceId, sig);
...
export async function cacheAudio(text, audioBuffer, voiceId, sig) {
  try {
    const supabase = getSupabaseAdmin();
    const hash = hashText(text, voiceId, sig);
