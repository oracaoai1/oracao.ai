...
// Pagamento único (pacotes de Velas). externalReference carrega o vínculo.
export async function createOneTimePayment({ customerId, value, description, externalReference }) {
...
// Altera o valor das próximas cobranças de uma assinatura existente.
export async function updateSubscriptionValue(subscriptionId, value, description) {
  return asaasFetch(`/subscriptions/${subscriptionId}`, {
    method: "PUT",
    body: { value, description, updatePendingPayments: true },
  });
}

// Cancela uma assinatura (usado ao excluir a conta do usuário).
