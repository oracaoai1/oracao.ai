...
function renderConteudo(texto) {
  ...
  texto.split('\n').forEach((linha, i) => {
    ...
    if (linha.startsWith('**') && linha.endsWith('**')) {
      els.push(
        <h3 key={i} className="artigo-secao">
          {linha.slice(2, -2)}
        </h3>
      );
      return;
    ...
    const inline = partes.map((p, j) =>
      p.startsWith('**') && p.endsWith('**') ? (
        <strong key={j} style={{ color: 'var(--gold)', fontWeight: 700 }}>
          {p.slice(2, -2)}
        </strong>
      ) : (
        p
    ...
    const ehVerso =
      linha.length < 120 && !linha.startsWith('**') && !/[.?!:]$/.test(linha);

    els.push(
      <p key={i} className={ehVerso ? 'artigo-verso' : ''}>
        {inline}
      </p>
    );
  });
